...
with tab3:
    ...
    if generate_btn and seed_start.isdigit() and seed_target.isdigit():
        ...
        st.rerun()
# ==========================================
# DATABASE MANAGEMENT FEATURE (ADDED)
# ==========================================
st.markdown("---")
with st.expander("⚙️ Database Management Tools", expanded=False):
    st.markdown("### 👤 Remove Player / UID")
    uid_input = st.text_input("Enter UID to delete from database:", key="mgmt_delete_uid_input")
    
    if st.button("❌ Delete UID from All Tables", use_container_width=True):
        if uid_input:
            try:
                import sqlite3
                import os
                if os.path.exists(DB_FILE):
                    conn = sqlite3.connect(DB_FILE)
                    cursor = conn.cursor()
                    # Automatically find all tables in the database dynamically
                    cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
                    tables = [row[0] for row in cursor.fetchall()]
                    total_deleted = 0
                    
                    # Search and delete matching UID from any column across all tables
                    for table in tables:
                        cursor.execute(f"PRAGMA table_info('{table}');")
                        columns = [col[1] for col in cursor.fetchall()]
                        for col in columns:
                            try:
                                cursor.execute(f"DELETE FROM '{table}' WHERE {col} = ?;", (uid_input,))
                                total_deleted += cursor.rowcount
                            except Exception:
                                pass
                    conn.commit()
                    conn.close()
                    st.success(f"Successfully processed removal! Removed instances of UID '{uid_input}' (Affected rows: {total_deleted})")
                else:
    ...
    if st.button("🔥 Nuke & Delete Entire Database File", use_container_width=True):
        try:
            import os
            if os.path.exists(DB_FILE):
                os.remove(DB_FILE)
                st.success(f"Database file '{DB_FILE}' has been completely deleted. It will be recreated freshly on the next operation.")
            else:
