...
st.set_page_config(page_title="Recon Engine: Hyper-Async Proxied", layout="wide")

st.title("⚡ OSINT Graph Reconnaissance (Hyper-Async Engine)")
st.write("Overhauled Event-Loop Core with dynamic distributed Proxy Swarm capabilities.")
...
with col1:
    ...
    batch_size = st.slider("Micro-Batch Size (Concurrent Tasks):", 5, 100, 30)
    
    st.subheader("🌐 Network Proxy Configuration")
    proxy_input = st.text_area(
        "Paste Proxies (One per line):",
        placeholder="http://user:pass@ip:port\nhttp://user2:pass2@ip2:port2",
        height=120,
        help="Paste a single backconnect rotating proxy URL or an entire list of individual proxy endpoints."
    )
    
    start_btn = st.button("🚀 Launch Proxied Swarm", use_container_width=True, type="primary")
...
async def fetch_friends_async(session, user_id, proxy_list):
    """Fires non-blocking network calls multiplexed over proxy nodes"""
    url = f"https://friends.roblox.com/v1/users/{user_id}/friends"
    
    # Selection algorithm picks from proxy array if loaded
    selected_proxy = random.choice(proxy_list) if proxy_list else None
    
    try:
        async with session.get(url, proxy=selected_proxy, timeout=6) as response:
        ...
        # Proxy connection failures or timeouts are automatically flagged as a retry state
        return user_id, [], True
...
async def resolve_usernames_async(session, id_list, proxy_list):
    ...
    selected_proxy = random.choice(proxy_list) if proxy_list else None
    try:
        async with session.post(url, json={"userIds": id_list, "excludeBannedUsers": False}, proxy=selected_proxy, timeout=6) as response:
            if response.status == 200:
                data = await response.json()
...
async def run_engine(s_id, t_id, max_concurrent, proxy_pool):
    ...
    if proxy_pool:
        log(f"[SYSTEM] Swarm armed with {len(proxy_pool)} external proxies.")
    else:
        log("[SYSTEM] WARNING: Running raw on local IP address. High risk of 429 throttling.")
    ...
    async with aiohttp.ClientSession() as session:
        while st.session_state.running:
            ...
            log(f"[EVENT-LOOP] Processing batch of {len(tasks)} parallel nodes...")
            results = await asyncio.gather(*tasks)
            api_calls += len(tasks)
            status_placeholder.info(f"API Calls Dispatched: {api_calls} | Mapped Target Tree Nodes: {len(start_visited) + len(target_visited)}")
            ...
            if rate_limit_tripped and not proxy_pool:
                log("[CRITICAL 429] Local IP throttled. Event-Loop forced to sleep for 60s...")
                for i in range(60, 0, -1):
                    status_placeholder.warning(f"Local Cooldown: {i}s remaining...")
                    await asyncio.sleep(1)
                continue
            
            # Tiny frame delay to keep user interface running fluidly
        ...
        if path_found:
            ...
            log(f"[SUCCESS] Target track confirmed. Resolving network handles...")
...
if start_btn and s_input.isdigit() and t_input.isdigit():
    ...
    st.session_state.logs = ["[SYSTEM] Reinitializing Distributed Event Loop Routing Engine..."]
    
    # Parse dynamic user proxies
